...
def generate_report(vulnerabilities: list[EnrichedVulnerability], report_date: date) -> str:
    ...
    for rank, vuln in enumerate(vulnerabilities, start=1):
        ...
        lines += [
            ...
            "",
            f"- **CVSS**: {vuln.cvss.base_score} ({vuln.cvss.base_severity})",
            ...
            f"- **랜섬웨어 연관**: {vuln.kev.known_ransomware_use}",
            "",
        ...
        lines.append("")
        lines.append(f"**참고 자료**: {', '.join(references) if references else '제공된 데이터에 없음'}")
        lines.append("")
